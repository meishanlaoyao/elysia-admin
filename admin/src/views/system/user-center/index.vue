    ...
    <div class="relative flex-b mt-2.5 max-md:block max-md:mt-1">
      <div class="w-112 mr-5 max-md:w-full max-md:mr-0">
        <div class="art-card-sm relative p-9 pb-6 overflow-hidden text-center">
          ...
          <img class="relative z-10 w-20 h-20 mt-30 mx-auto object-cover border-2 border-white rounded-full"
            :src="baseUserInfo?.avatar || defaultAvatar" />
          <h2 class="mt-5 text-xl font-normal">{{ baseUserInfo?.username }}</h2>
          <p class="mt-5 text-sm">专注于用户体验跟视觉设计</p>

          <div class="w-75 mx-auto mt-7.5 text-left">
            <div class="mt-2.5">
              <ArtSvgIcon icon="ri:mail-line" class="text-g-700" />
              <span class="ml-2 text-sm">{{ baseUserInfo?.email }}</span>
            </div>
            <!-- <div class="mt-2.5">
            ...
            <div class="mt-2.5">
              ...
              <span class="ml-2 text-sm">{{ baseUserInfo?.deptName }}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <ElRow>
              ...
              <ElFormItem label="性别" prop="sex" class="ml-5">
                <ElSelect v-model="form.sex" placeholder="Select" :disabled="!isEdit">
                  <ElOption v-for="item in system_user_sex" :key="item.dictValue" :label="item.dictLabel"
 ...
 */
const getDate = () => {
  fetchGetUserBasic().then(res => {
    baseUserInfo.value = res
