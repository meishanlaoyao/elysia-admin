import { pgTable, bigserial, varchar, jsonb, bigint, decimal, timestamp } from 'drizzle-orm/pg-core'; // bigint 保留用于 merchantId
...
export const businessOrdersSchema = pgTable(
    'business_orders',
    {
        ...
        userId: varchar('user_id', { length: 64 }).notNull(), // 下单用户ID（支持 UUID 等字符串类型）
