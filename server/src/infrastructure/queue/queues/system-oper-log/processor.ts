/**
 * SystemOperLog Processor（沙箱模式）
 * 每个任务在独立子进程中执行
 */
import type { SandboxedJob } from 'bullmq';
import { createTaskRegistry } from '@/infrastructure/queue/core/processor-utils';
import { registerSystemOperLogSandboxTasks } from '@/worker-sandbox/system-oper-log-tasks';

const { register, get } = createTaskRegistry();
registerSystemOperLogSandboxTasks(register);

export default async function processor(job: SandboxedJob) {
    const taskFn = get(job.name);
    if (!taskFn) throw new Error(`[SystemOperLog] 未找到任务: ${job.name}`);
