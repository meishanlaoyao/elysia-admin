 ...
 */
export async function IpRateLimitGuard(ctx: AppContext) {
    const limitInfo = ctx?.routeInfo?.meta?.ipRateLimit;
    if (!limitInfo) return;
 ...
 */
export async function IpRateLimitRecord(_ctx: AppContext) {
    return;
};