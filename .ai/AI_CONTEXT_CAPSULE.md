...
2. **No hardcoded enums:** align `system_dict_*`; missing → `server/database/sql/{module}-init.sql`
3. **Permissions ×3:** `route.ts` ↔ frontend auth ↔ SQL `permission`
4. **Menu SQL:** query DB (MCP) before INSERT; one merged SQL file
5. **Git read-only:** status/diff/log OK; no add/commit/push unless user asks
6. **Optimize on demand:** no default indexes/Redis for CRUD boilerplate
