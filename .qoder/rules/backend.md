---
description: Backend module rules (Elysia + Bun) — file structure, repository usage, code patterns
globs:
  - server/src/**/*.ts
alwaysApply: false
---
...
**Cross-module `handle`:** system domain (auth, user, role, menu, dept) may import exported functions from each other; new business modules should stay self-contained.
...
| `handle.ts` | repository, QueryBuilder, shared, infrastructure; system domain cross-imports | raw SQL, direct pg, new Elysia in handle |
