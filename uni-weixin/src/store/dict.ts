...
import { findAllData } from '@/api/dict'
import type { FindDictDataDto } from '@/api/types/Idict'

export const useDictStore = defineStore(
    'dict',
    () => {
        /** 字典数据 */
        const dictData = ref<Record<string, FindDictDataDto[]>>({})

        /** 正在请求中的字典类型，防止重复请求 */
        const pendingRequests = new Map<string, Promise<FindDictDataDto[]>>()
         ...
         * @returns 字典数据数组
         */
        const fetchDictType = async (dictType: string): Promise<FindDictDataDto[]> => {
            if (pendingRequests.has(dictType)) {
                return pendingRequests.get(dictType)!
            }
            const promise = findAllData(dictType)
                .then(res => {
                    const data = res || []
        ...
        const getDictData = (arr: string[]) => {
            const result = reactive<Record<string, FindDictDataDto[]>>({})
            arr.forEach(type => {
